...
contract Foxcon2022 is ERC721, ERC721URIStorage {
    ...
    function mint() public payable {
        require(totalSupply() + 1 <= MAX_SUPPLY, "Purchase would exceed max supply of Tickets");
        require(ticketPrice <= msg.value, "Ether value sent is not correct");

        uint mintIndex = totalSupply();
        if (totalSupply() < MAX_SUPPLY) {
          _createTicket(msg.sender, mintIndex, "QmcriZCeovxW61mYY6hNfYX1bmLh9gFUD294jhqUKkAUrk");
        }
    }
